...
$$ tf_idf{t,d} = f_{t,d} \times invf_{t,d}  $$
...
The TF-IDF value grows proportionally to the occurences of the word in the TF, but the effect is balanced by the occurences of the word in every other document (IDF).

## 3. Measuring similartiy between documents 
...
The pre-processing will be similar to the one developped in the previous article. We'll use the `preprocess` function. This pipeline is only an example that happened to suit my needs on several NLP projects. It has many limitations, including the face that it only handles english vocabulary.
...
So far, we used a self-defined function. As you might guess, there is a Sk-learn way to do this :)
