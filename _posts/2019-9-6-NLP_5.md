...
`train` now contaons only 6 observations, 3 from each class :
...
At that point, we need to get the aveage embeddings by first getting the token id in the model vocabulary, and then getting the corresponding word token, before finally average over the whole sentence.
