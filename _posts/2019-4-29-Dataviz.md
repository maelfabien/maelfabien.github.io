...
![image](https://maelfabien.github.io/assets/images/viz_1.png)
...
![image](https://maelfabien.github.io/assets/images/viz_2.png)
...
- The channels that show magnitude information are a good for ordered data. (how much is it ? How much longer ? How much wider ?…)
- The channels that show identity information are good with  categorical data. (What is it ? Where is it ? Is it a line ?…)

Usually, a mark represents an item. (In a table dataset for example). In a network dataset, mark can represent an item (node) , or a link between items. There are 2 types of link marks :
...
![image](https://maelfabien.github.io/assets/images/viz_3.png)

The uman visual system does not process 2 different channels the same way. We decide which channel to use according to 2 principles :
...
- **Effectiveness** : the most important attributes should be encoded with the most effective channels in order to be most noticeable

What channel is effective when it comes to visual encoding ?

![image](https://maelfabien.github.io/assets/images/viz_4.png)
...
- Consume Information, that has already been generated as data stored
    ...
    - Discover : Use Vis to find new knowledge that was not previously known :
        ...
        - Verify or disconfirm existing hypothesis
- Enjoy : Curiosity (Infographic on a blog post for example)

At **middle level**, Search is use to localize known or unknown data.
...
- Explore : Users are not sure of location.
...
- **Find anomalies** : Identify anomalies in a set of data cases with respect to a given relationship / expectation
...
In InfoVis, the user first understands information displayed on a graph, and then starts to work around and manipulate the data. This part is called the interaction. There are several kind of interactions :
...
- a parallel extraction of low-level properties of scene, called **preattentive** process
...
- A categorical processing is easy to find when unique
- We, as humans, cannot process both color and shape preattentively
