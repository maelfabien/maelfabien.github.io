...
We create a global variabled, `d3` that we will use to interact with the D3 system. If we run this part and inspect the code of the HTML page, we sould see something that has the following structure :
...
- we don't put anything in front on string columns, since it's the default type
...
This function tells us to append new "rects" to our canvas and what function to apply to all "rects". We then need to add attributes to this function, including which data it should map on the x-axis and on the y-axis.
...
> **Conclusion** : That's it ! I hope this introduction to D3 was interesing. I have so far used D3 on 2-3 projects, and even though some things are tricky to understand, it's a powerful tool. I highly recommend checking D3's documentation, it's absolutely fantastic, well illustrated and well written.
