...
Given a sequence of characters, the aim of tokenization is to cut the sentence into pieces, called tokens. Tokenization consists in splitting large chunks of tesxt into sentences, and sentences into a list of single words, also called tokens. This step, also referred to as segmentation or lexical analysis, is necessary in order to perform further processing.
...
- using Machine Learning on large labelled corpus :
...
- Inflectional form is a change in a word that shows a change in the way it is used in the sentence
- Morphological derivation is the process of forming a new word from an existing one by adding prefix or suffix for example.
