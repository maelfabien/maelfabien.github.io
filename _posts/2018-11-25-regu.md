...
In this short article, we arre going to cover the concepts of the main regularization techniques in deep learning, and other techniques to prevent overfitting.
...
![image](https://maelfabien.github.io/assets/images/overfit.png)
...
![image](https://maelfabien.github.io/assets/images/train_split.png)
...
![image](https://maelfabien.github.io/assets/images/class_imb.png)

Class weights directly affect the loss function, by modifying the amount of data of each class sent in each batch. Therefore, we have a equivalent amount of data from each class sent in each batch. This however requires that the amount of data in the minor class remains sufficiently important so that there is no overfitting on 200 examples being reused all the time for example.
...
![image](https://maelfabien.github.io/assets/images/dropout.png)
...
Early Stopping is a way to stop the learning process when you notice that a given criteria does not change over a series of epochs. For example, if we want the validation accuracy to increase, and the algorithm to stop if it does not increase for 10 periods, here is how we would implement this in Keras :
...
This technique is quite interesting and can really help your network. Even if you have an "Adam" or "RMSProp" optimizer, it might happen that your network gets stuck at some point on a plateau. In such case, it might be useful to reduce the learning rate and try to get those extra percent of accuracy. Indeed, a learning rate a bit too large might simply mean that you overshoot the minimum and are kind of stuck close to a minimal point.
