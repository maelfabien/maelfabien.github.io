...
You might have already heard of Fairseq, a sequence-to-sequence toolkit written in PyTorch by FacebookAI. One of the most common applications of Fairseq among speech processing enthousiasts is wav2vec (and all the variants), a framework that aims to extract new types of input vectors for acoustic models from raw audio, using pre-training and self-supervised learning. 
...
This additional layer $$ q : Z \to \tilde{Z} $$ that takes $$ Z $$, a dense representation learnt by the first CNN, and outputs a discrete representation, using either K-means clustering or the Gumbel-Softmax as constraints in a Vector Quantized Variational Autoencoders. 
...
The discrete speech representation is then fed to train a BERT architecture. BERT is a pre-training approach for NLP tasks, which uses a transformer encoder model to build a representation of text. Transformers uses self-attention to encode the input sequence as well as an optional source sequence.
...
Authors used wav2letter as accoustic model and trained for 1,000 epochs on 8
GPUs for both TIMIT and WSJ. Language models used were the a 4-gram KenLM language model and a character based convolutional language model.
...
One last important work, although results were not that good, is the sequence-to-sequence model that authors explain in a small paragraph. Rather than training BERT on discretized speech, and inputing it in the acoustic model, one could solve speech recognition as an end-to-end task. 

This is solved taking the discrete speech representation as an input to a Transformer architecture. Hence, no acoustic model is required. No language model or data augmentation were used, which might explain the limited results. But this work is an important step towards what the next papers of this series explore.
...
Authors added a randomly initialized linear projection on top of the context network (transformer) into $$ C $$ classes representing the vocabulary of the task.
...
Finally, authors mention that self-training is likely complimentary to pre-training and their combination may yield even better results. However, it is not
...
In their paper, authors explore the complementarity of self-training and pre-training.

The pre-training approach adopted so far relied of self-supervised learning. This whole block will now be referred as unsupervised pre-training. This paper introduces the notion of self-training.
...
The final model trains a Transformer-based sequence to sequence model with log-Mel filterbank inputs after pseudo-labeling using wav2letter++. It uses a 10k word piece output vocabulary computed from the training transcriptions if the whole
Librispeech training set is used as labeled data.

## b. Results

Results acheived on only 10 minutes of data are even better than wav2vec 2.0.

![image](https://maelfabien.github.io/assets/images/wav_12.png)

Finally, authors explore to what extent self-training and pre-training are complementary. According to the table below, it appears when respecting a ratio of 8.6 times more unlabeled speech that labeled one, self-training keeps improving results by more than 7% on average.
...
- that wav2vec can be used as a new representation of speech, which can itself be used an inputs to other downstream tasks such as speech recognition
- that for ASR, wav2vec, i.e. pre-training using a self-supervised learning on large amount of unlabeled data can help the model performance on limited amout of data later
...
Well, this is an open research question. I am myself really interested in exploring this. This is what I have currently found:
...
Speech representations can be used for several downstream tasks. I am rather convinced that it is a matter of time before theses representations, learnt from raw waveforms, approach SOTA or even beat it on other tasks such as speaker identification, voice activity detection, language identification, pathological speech detection... I see it as a very powerful tool.
