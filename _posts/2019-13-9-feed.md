...
> "Notation in Deep Learning is important", Tony Parker
...
We will focus here on the last part of the Neural Net :

![image](https://maelfabien.github.io/assets/images/nn_21.png)

Let's break down the first derivative :
...
Where $$ h_i $$ is the value taken by the output of a neuron at each step. For example, in the first step :

$$ h_1 = W_{11}^{(1)} x_1 +  W_{21}^{(1)} x_2 +  W_{31}^{(1)} $$

$$ h_2 = W_{12}^{(1)} x_1 +  W_{22}^{(1)} x_2 +  W_{32}^{(1)} $$

$$ h = W_{11}^{(2)} \sigma(x_1) +  W_{21}^{(2)} \sigma(x_2) +  W_{31}^{(2)} $$
...
Using the expression of $$ h $$ we just defined, we can compute the derivative with respect to $$ h_1 $$ :

$$ \frac{\partial h}{\partial h_1} = W_{11}^{(2)} \sigma'(h_1) $$ where $$ \sigma'(x) $$ is the derivative of the sigmoid.
