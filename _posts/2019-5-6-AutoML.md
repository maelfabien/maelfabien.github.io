...
We can now make a prediction using the leader model and assess its performance on the test set using the F1-Score. We simply need to transform the h2o objects back to pandas data frames to compute the scores.
...
Which heads on my side `0.8393782383419689`. Since I ran this notebook locally, I would expect a 6 hours runtime on GPUs to outperform this. We can then save the best model :
