...
![image](https://maelfabien.github.io/assets/images/spark_logo.png)
...
![image](https://maelfabien.github.io/assets/images/comp.png)
...
![image](https://maelfabien.github.io/assets/images/dag.png)
...
- DataFrame : DataFrames are layers on top of RDDs. It looks like a Pandas DataFrame, with columns, but is indeed an RDD of rows.
- DataSet : DataSets are layers on top of DataFrames which allow to specify types for columns.
...
There are some key operations one can do in Spark that are really interesting for parallel computation.

### Shuffling

Since we work on a multi-node system, shuffling makes an efficient redistribution of our data on the working nodes. By shuffling the data accross the cluster nodes, we are able to execute tasks much faster. 

![image](https://maelfabien.github.io/assets/images/shuffling.png)

However, this operation requires to write data on disk, and is therefore really expensive in terms on time. 

### Persistence

Persistence is the operation that stores your dataset into fast RAM memory.
...
- If you persist your clients dataset, each node will put his part of the dataset into cache memory.
...
> Conclusion : All those concepts still need illustration, but it's the purpose of the next article. I hope this high level overview was clear and helful. I'd be happy to answer any question you might have in the comments section.
