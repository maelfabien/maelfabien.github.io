...
Dans le menu latéral, l'utilisateur peut générer des données selon une loi initiale. Il peut décider des paramètres de la loi, et d'une perturbation à ajouter. Il peut ensuite ajouter une tolérance en termes de somme des résidus au carré, et allouer un budget de temps pour l'algorithme de modélisation. Les données sont ensuite généréées automatiquement, et l'algorithme de modélisation est lancé. La meilleure loi identifiée, les paramètres et le tempx d'exécution sont alors affichés.
...
Le code de l'application Streamlit est disponible sur Github. Pour lancer l'application Streamlit, il suffit de lancer:
...
A travers cet article, nous avons identifié les enjeux de la modélisation de distribution, et présenté une méthode automatisée pour identifier la loi optimale et les meilleurs paramètre par maximum de vraisemblance. Avec l'appui d'applications interactives comme Streamlit, l'exploration de la sortie de l'algorithme est bien plus intéractive.
